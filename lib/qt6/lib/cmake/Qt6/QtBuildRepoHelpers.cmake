...
function(qt_build_internals_set_up_system_prefixes)
    if(APPLE AND NOT FEATURE_pkg_config)
        # Remove /var/empty/local and other paths like that which CMake considers as system prefixes on
        ...
        #
        # Note that we can't remove /var/empty as a system prefix path, because many programs won't be
        # found then (e.g. perl).
        ...
        list(REMOVE_ITEM CMAKE_SYSTEM_PREFIX_PATH
            "/var/empty/local" # Homebrew
            "/var/empty/homebrew" # Apple Silicon Homebrew
            "/var/empty/X11R6"
            "/var/empty/pkg"
            "/opt"
            "/sw" # Fink
            "/var/empty/local" # MacPorts
        )
...
function(qt_build_internals_disable_pkg_config_if_needed)
    ...
    # tell CMake to consider paths like /var/empty/local (Homebrew) as system paths when looking for
    # packages.
    ...
    # only be found via FooConfig.cmake files which means /var/empty/local should be in the system prefix
    # path.
